 ...
 */
Ext.define('Ux.datepicker.DatePicker', {
	extend: 'Ext.Panel',
	...
	monthMarkup: function(month, year) {
		...
		this_month += '<th class="goto-prevyear">' + Date.getShortDayName((0 + x) % 7) + '</th>';
		this_month += '<th class="goto-prevmonth">' + Date.getShortDayName((1 + x) % 7) + '</th>';
		this_month += '<th>' + Date.getShortDayName((2 + x) % 7) + '</th>';
		this_month += '<th><span class="month-year-label">' + Date.monthNames[month] + ' ' + year + '</span>' + Date.getShortDayName((3 + x) % 7) + '</th>';
		this_month += '<th>' + Date.getShortDayName((4 + x) % 7) + '</th>';
		this_month += '<th class="goto-nextmonth">' + Date.getShortDayName((5 + x) % 7) + '</th>';
		this_month += '<th class="goto-nextyear">' + Date.getShortDayName((6 + x) % 7) + '</th>';
		this_month += '</tr>';
		this_month += '</thead>';
	...
	setToday: function() {
		var date = new Date();
		this.element.select('td[datetime="' + date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + '"]').addCls('today');
	},
