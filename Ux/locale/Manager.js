...
Ext.define('Ux.locale.Manager', {
    ...
    applyLocales : function(startingAt) {
        var queryString = [];
        if (startingAt)
            queryString.push(startingAt + ' ');
        queryString.push('component[enableLocale]');

        var cmps     = Ext.ComponentQuery.query(queryString.join('')),
            c        = 0,
            cNum     = cmps.length,
    ...
    updateLocale : function(locale, callback) {
        var me = this;
